...
const UseReducerExample = () => {
  ...
  // bunuda statelere aktarıyor ve gelen veriyi de dispatch ile dağıtıyoruz.
  const [state, dispatch] = useReducer(reducer, initialState);

  const { loading, error, userImage } = state;

  // APİ'den VERİYİ GETİRECEK FONKSİYONU TANIMLADIM.
  const getUser = async () => {
  ...
  return (
    <div>
      <div>
        ...
        {error && <h2>{error}</h2>}
        {userImage && (
          <img width="15%" src={userImage.picture?.medium} alt="img" />
        )}
        {userImage && (
          <p>
            {" "}
            {userImage.name.first} {userImage.name.last}
          </p>
        )}
      </div>
    </div>
